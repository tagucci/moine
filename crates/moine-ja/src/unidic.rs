...
pub struct DictionaryReadingSegment {
    ...
    pub reading: String,
    /// Whether the segment came from the dictionary or direct surface fallback.
    pub source: DictionaryReadingSegmentSource,
}

/// Source of one Japanese reading-path segment.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum DictionaryReadingSegmentSource {
    /// Segment was backed by a dictionary entry.
    Dictionary,
    /// Segment was copied from direct kana/romaji surface fallback.
    Direct,
}
...
pub fn romaji_paths_from_reading_paths(
    ...
    romaji_paths_from_segmented_readings(
        paths
            .iter()
            .map(|path| path.segments.iter().map(romaji_segment_input)),
    )
...
fn romaji_segment_input(segment: &DictionaryReadingSegment) -> (&str, RomajiSegmentMode) {
                    ...
                    if !suffix_paths[end].is_empty() {
                        ...
                        for suffix in &suffix_paths[end] {
                            ...
                            segments.push(DictionaryReadingSegment {
                                ...
                                source: DictionaryReadingSegmentSource::Direct,
                            });
                            segments.extend(suffix.segments.iter().cloned());
...
fn direct_fallback_end(
    ...
    while end < char_len {
        ...
        if !can_build_direct_romaji_path(surface) {
            break;
        }
        end += 1;
    }
